<?php
	...
	function generateRow($from, $to, $conn, $deduction){
		...
		// pree   $sql = "SELECT *, sum(num_hr) AS total_hr, attendance.employee_id AS empid FROM attendance LEFT JOIN employees ON employees.id=attendance.employee_id LEFT JOIN position ON position.id=employees.position_id WHERE date BETWEEN '$from' AND '$to' GROUP BY attendance.employee_id ORDER BY employees.lastname ASC, employees.firstname ASC";
		...
		while($row = $query->fetch_assoc()){
			$empid = $row['empid'];
	      	...
	      	// pree $casql = "SELECT *, SUM(amount) AS cashamount FROM cashadvance WHERE employee_id='$empid' AND date_advance BETWEEN '$from' AND '$to'";
    ...
    $content = '';  
    $content .= '
      	<h2 align="center">Captain Cook Ltd Employee with Cash Advance Payroll</h2>
